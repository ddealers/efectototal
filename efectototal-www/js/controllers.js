...
.controller('AppCtrl', function($scope, $state, Videos) {
	...
	$scope.share = function(){
		FB.ui({
			method:'apprequests',
			message:'¡Ejercítate de una forma divertida sintiendo el Efecto Total!'
		});
...
.controller('LoginCtrl', function($scope, $state, OpenFB, User) {
	...
	$scope.facebookLogin = function () {
		$scope.loading = true;
		FB.login(registerAndLogin, {scope:'email,user_friends'});
	};
})
	...
	$scope.share = function(sm){
		switch(sm){
			...
			*/
			FB.ui({
				method:'share',
				href:'¡Ejercítate de una forma divertida con esta rutina de Efecto Total! http://efectototal.com/videos/cat/'+$stateParams.cat
			});
			break;
			case 'tw':
	...
	*/
	...
	Videos.byCategory($stateParams.cat).then(
		function(data){
			...
			//video[0].addEventListener('click', $scope.onInit, false);
...
.controller('VideoCtrl', function($scope, $state, $stateParams, $interval, $ionicModal, $ionicNavBarDelegate, History, Playlist, CaloricCounter, Videos, User){
	...
	$scope.share = function(sm){
		switch(sm){
			...
			*/
			FB.ui({
				method:'share',
				href:'¡Ejercítate de una forma divertida con esta rutina de Efecto Total! http://efectototal.com/videos/showVideo/'+$stateParams.video
			});
			break;
			case 'tw':
	...
	*/
	...
	$scope.onInit = function(){
		...
		//video[0].pause();
		//onConfirm(0);
		//confirm('Comenzar a contar calorías. Todas tus calorías se contarán para tus retos.', onConfirm, 'Contador', ['Comenzar','Cancelar']);
...
.controller('NewsfeedCtrl', function($scope, $state, History){
	...
	$scope.loadMore = function(){
		...
		FB.api('/me/?fields=friends',function(data){
			console.log(data, data.friends, data.friends.data);
			var _i = 0, _results = [];
			...
			History.newsfeed(_results.toString(), page).then(function(data){
				...
				for(a in data){
					var activity = data[a];
					if(activity.data && activity.data.indexOf(',') > -1){
						activity.data = activity.data.split(",");
					}
				}
				$scope.activities = $scope.activities.concat(data);
